  ...
  return (
    <div className='flex flex-col justify-center items-center h-screen' >
      {selectedCityData.map((data) => (
          <div key={data.id} style={{backgroundImage: `url(${data.largeImg})`, backgroundRepeat: 'no-repeat', backgroundSize:"cover", width: "100%"}} className="h-full text-white">
            <div className='grid grid-cols-1 sm:grid-cols-2 justify-center items-center h-full'>
              ...
              <div className='flex flex-col pt-24 sm:bg-neutral-700 sm:bg-opacity-80 h-full p-8'>
                <p className='text-xl font-semibold italic'>{data.name}</p>
                <p>{data.description}</p>
                <h4 className='text-xl font-semibold pt-10 italic'>Tours</h4>
              ...
              <div className='flex justify-center items-center'>
                <h1 className='bg-neutral-700 bg-opacity-40 rounded-xl p-2 text-5xl font-semibold italic'>{data.name}</h1>
              </div>
            </div>
          </div>
        ))}
